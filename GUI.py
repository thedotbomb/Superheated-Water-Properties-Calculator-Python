...
#titlelabel = Lable(root, text= "SUPPER HEATED WATER PROPOERTIES CALCULATOR")

templabel = Label(root, text= "Enter Temperature(100 to 1300) in C:")
templabel.grid( row = 0, column = 0, sticky = W)
tempentry = Entry( root)
tempentry.grid( row = 0 , column = 1 , sticky = W)

pressurelabel = Label(root, text = "Enter Pressure(.01 to 60) in MPa:")
pressurelabel.grid( row = 1, column =0, sticky =W)
pressureentry= Entry( root)
pressureentry.grid( row = 1, column =1, sticky = W)





def super_heated_water_program():

    
    tt = tempentry.get()
    temperature=float(tt)+ .001
    pp = pressureentry.get()
    pressure = float(pp) + .001
    ...
    #### First double file reader 
        ...
        finalV , finalE , finalU , finalH = quality_interpolate(Pressure_higher, Pressure_lower, pressure, Temp_higher, Temp_lower, temperature, vh, uh, hh, sh, vl, ul, hl, sl , vH, vL, uH , uL ,hH, hL, sH, sL ) 
    ...
    if finalU == "Out of Range":
        final = "Out of Range"
        an =Text(root, width=35, height=5,wrap= WORD)
        an.grid(row = 5 , column=0)
        an.delete(0.0, END)
        an.insert(0.0, final)
    else:
        final =  "Internal", "Energy:",round(finalU,4), "kJ/kg", "Specific","Enthalpy:",round(finalH,4),"kJ/kg", "Specific","Entropy:",round(finalE ,4),"kJ/kgK","Specific", "Volume:",round(finalV,4),"m^3/kg"                   
        an =Text(root, width=35, height=5,wrap= WORD)
        an.grid(row = 5 , column=0)
        an.delete(0.0, END)
        an.insert(0.0, final)
    
    #an.insert(0.0, finalV)

     
button = Button(root, text = "Calculate ", command = super_heated_water_program )
button.grid( row = 3)
